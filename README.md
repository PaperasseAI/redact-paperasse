...
- `cargo fmt --all --check`, `cargo clippy --workspace --all-features --all-targets -D warnings`, `cargo test --workspace --locked` — native host. **95/95 tests pass.**
...
15. **A redacted PDF comes back as pictures of a page, and one page came back at half the resolution of the other.** Measured on a real two-page Kbis: the input had 30 font objects of live vector text; the output had none, replaced by full-page bitmaps at ~85 DPI (page 1) and ~147 DPI (page 2). The boxes themselves land correctly — that was checked by rendering the output rather than trusting a calculation that said they should be nearly 2x out. Two separate defects sit underneath. First, `--dpi` does not reach PDF rendering at all: 150 and 300 produce byte-identical page dimensions, so the flag silently did nothing here and its help text now says so. Second, and the real fix, a PDF that already carries a text layer should never be rasterized — the spans are known exactly without pixels, so redaction should cover them with vector rectangles and *remove the underlying text*, leaving every other glyph untouched. Rasterizing must stay the path for genuinely scanned PDFs. Whatever replaces it needs a test that extracts text from the output and asserts the redacted string is absent: a black rectangle drawn over text that is still selectable underneath is the classic redaction failure that leaks documents.
